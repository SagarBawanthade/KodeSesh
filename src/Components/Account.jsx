...
const Account = () => {
  ...
  const fetchUserDetails = async () => {
    try {
      ...
      const response = await axios.get(`http://65.0.135.137:5000/api/auth/user/${userId}`, {
        headers: {
  ...
  const handleSubmit = async () => {
    try {
      ...
      // Use the correct API endpoint based on your server routes
      const response = await axios.put(`http://65.0.135.137:5000/api/auth/user/${userId}`, updateData, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
